...
async function findLatestResultFile(sessionId) {
  try {
    ...
    const recentFiles = [];
    for (const file of files) {
      if (file.endsWith('.json') && file.includes('foscos_result')) {
...
// Create a Python runner script that directly calls the main function
...
def main():
    ...
    try:
        ...
        if not license_result or "error" in license_result:
            log_message(session_id, f"License extraction failed: {license_result}")
            result = {
                "session_id": session_id,
                "url": url,
                "timestamp": datetime.now().isoformat(),
                "status": "error",
                "error": "Failed to extract license information",
...
async function cleanupOldResults() {
  try {
    ...
    const resultFiles = files
      .filter(file => file.endsWith('.json') && file.includes('foscos_result'))
      .map(file => ({
        name: file,
...
async function runPythonScript(sessionId, url) {
  ...
  try {
    ...
    const pythonCommand = process.env.PYTHON_PATH || (process.platform === 'win32' ? 'python' : 'python3'); // Fixed typo: PYTHON_PAT -> PYTHON_PATH
